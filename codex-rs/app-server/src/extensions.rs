    ...
    fn continue_run<'a>(
        ...
        Box::pin(async move {
            ...
            let continuation = ContextualUserFragment::into(InternalModelContextFragment::new(
                ...
                format!(
                    "Continue Autonomous Stateful run {} toward its explicit goal. Start from the current World State and do not repeat completed work or reopen unchanged host-audited sourceVerified evidence merely to confirm it. Read source only for stale or unchecked evidence, missing detail, exact wording/format/code, active conflicts or material uncertainty, or explicit user-requested verification. Continue useful authorized work without routine checkpoints. Record a semantic obligation update when learning, strategy, uncertainty, or readiness materially changes. Finish every durable knowledge, relationship, steering, verification, and obligation write before marking the run completed; completion must be the final Stateful mutation. Complete with an evidence-grounded result when the goal is satisfied, or block only for a genuine authorization or external-state boundary.",
                    request.run_id
                ),
            ));
