...
async fn autonomous_run_continues_after_idle_until_the_model_completes_it() -> Result<()> {
    ...
    let project: ProjectCreateResponse = server
        .request(|request_id| ClientRequest::ProjectCreate {
            request_id,
            params: ProjectCreateParams {
                name: "Unattended investigation".to_string(),
                roots: Vec::new(),
                metadata: None,
                idempotency_key: "autonomous-project".to_string(),
            },
        })
        .await?;
    let thread = server
    ...
    let started: StatefulRunStartResponse = server
        .request(|request_id| ClientRequest::StatefulRunStart {
            ...
            params: StatefulRunStartParams {
                project_id: project.project.id,
                thread_id: thread.thread.id.clone(),
                goal: "Complete the investigation across as many turns as needed.".to_string(),
                mode: StatefulWorkflowMode::Autonomous,
                budget: StatefulRunBudget {
                    max_continuations: 2,
                    max_elapsed_seconds: 3_600,
                },
                idempotency_key: "autonomous-run".to_string(),
            },
        })
        .await?;
    let response_log = responses::mount_sse_sequence(
    ...
    server
        .start_turn_and_wait_for_completion(TurnStartParams {
            thread_id: thread.thread.id.clone(),
            input: vec![UserInput::Text {
                text: "Begin the investigation.".to_string(),
                text_elements: Vec::new(),
            }],
            ..Default::default()
        })
