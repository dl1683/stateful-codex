...
impl ProjectIntelligenceStatus {
    ...
    fn render(&self) -> String {
        ...
        append_line(
            &mut body,
            "Start from accumulated project intelligence. Root entries labelled verification=sourceVerified and evidence=current were host-audited this turn against their cited sources; use them as established premises and do not reopen those sources merely to confirm them. User-confirmed entries are established user-supplied premises. Read raw source only when evidence is stale, unavailable, or uncheckedThisTurn; when required detail is absent from state; when exact wording, format, or code must be reproduced; when active findings conflict or material uncertainty remains; or when the user requests fresh verification. After reading missing or changed evidence, update durable state. Bound investigation to the requested outcome: do not turn one criterion or decision dimension into an overall project determination. Do not query deeper state, search by every known filename, or reread the corpus merely to repeat adequate root knowledge. Use a focused deeper-blackboard or context-map query only when the root or its routes are insufficient.",
        );
        append_line(
            &mut body,
