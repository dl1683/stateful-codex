...
2. **Durable state does not yet provide adequate working continuity after compaction.** Each compacted replacement rehydrated one fresh Stateful project packet, but only 30 sparse blackboard entries existed after ten deep questions and completed-run results/final obligations were not projected into later model context. Conclusions outside that state depended on the provider's opaque compaction output; the subsequent rebuild reading shows that the installed state plus summary was insufficient. The empty `retained_context.verified_answers` field is not evidence of lost project conclusions: it records host-verified `request_user_input` replies for Guardian authorization and was expected to be empty in these autonomous read-only turns.
3. **Net effect: the product thesis is inverted.** The product intent says Stateful Codex should cut rereading and lifetime tokens, so a session should get cheaper as understanding accumulates. Here it got relatively more expensive, while overall quality came out level (Droid preferred ordinary on 6 of 10 queries; the summed rubric scores were 208 vs 210). Stateful's recorded state was not actively retrieved in later turns: reuse happened only through the injected summary, and a finding present in that summary went unused.
...
- Compaction plus deterministic Stateful rehydration preserves enough material understanding that the thread continues from what it concluded rather than rebuilding it through raw reads. Project conclusions belong in bounded durable project state, not in Guardian's authorization-specific retained-context families.
